...
// ensureClientTables applies migrations/0031 when the database has not had
...
func ensureClientTables(t *testing.T, pool *postgres.Pool) {
	...
	sql, err := os.ReadFile("../migrations/0031_client_devices.up.sql")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Raw().Exec(testCtx(t), string(sql)); err != nil {
		t.Fatalf("applying 0031_client_devices: %v", err)
	}
	// Recorded as `admin migrate` records it, so a later migrate does not
	// apply it a second time.
	if _, err := pool.Raw().Exec(testCtx(t), `INSERT INTO schema_migrations (version, applied_at) VALUES ('0031_client_devices', now())
		ON CONFLICT (version) DO NOTHING`); err != nil {
		t.Fatalf("recording 0031_client_devices: %v", err)
	}
}
