...
func (h phaseHandlers) bindWork() map[string]commandFunc {
	return map[string]commandFunc{
		...
		// The scheduler's dispatch payload, when a shift's time is up.
		"job.finish_shift": func(ctx context.Context, env *envelope.Envelope) (*presenter.Response, error) {
			var req handlers.FinishShiftRequest
			if err := decode(env, &req); err != nil {
				return nil, err
			}
			return h.jobs.FinishShift(ctx, env.Metadata, req)
		},
...
// swap, and both run on the game clock (config game.time_scale).
func newWorkHandlers(
	...
	jobs := handlers.NewJobsHandler(uow, uuidGenerator{}, msgs, registry, cities, policy, scale,
		handlers.DefaultPageSize, idempotencyTTL, nil)
	education := handlers.NewEducationHandler(uow, uuidGenerator{}, msgs, registry, cities, scale,
		handlers.DefaultPageSize, idempotencyTTL, nil)
