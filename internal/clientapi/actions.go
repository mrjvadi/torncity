...
// out: it is nothing the client could send.
func Actions(kb *presenter.Keyboard, policy *groups.Policy) []Action {
	out := []Action{}
	if kb == nil {
		return out
	}
...
func action(b presenter.Button, policy *groups.Policy) (Action, bool) {
	if b.URL != "" {
	...
	if command, args, ok := input.ParseAsk(data); ok {
		...
		return Action{Label: b.Text, Command: command, Args: payload,
			Input: &ActionInput{Field: spec.Field, Text: spec.Text}}, true
	}
	command, payload, err := routing.ParseCallbackData(data)
	if err != nil || !commands.FromPlayerCommand(command) {
		return Action{}, false
	}
	return Action{Label: b.Text, Command: command, Args: payload}, true
}
